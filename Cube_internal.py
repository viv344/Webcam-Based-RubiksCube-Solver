...
#stores state of cube as a dictionary, a solved cube has U: white D: yellow F: red B:orange L:green R:blue
#solving algorithm comprises of seven stages and includes functions that check that stage has been reached
#and functions that solves each stage
...
#creates a copy of the current state of cube by creating a copy dictionary and adding all elements of state
...
#function that rotates cube by swapping elements of select face arrays 
	def rotate(self, action,display=False):
		...
		#refers to cube_model.py, if display is True, then this rotation will be displayed in model
		if display:
			state = self._display_state
		else:
			state = self._state
...
#takes in the list of steps needed to solve the cub and rotates the cube object by those moves
	...
	#Function checks if white cross is finished with its adjacent pieces also being in the right place
	def is_stage_finish_white_edges(self):

		if not self._state['U'][0][1] == self._state['U'][1][0] == self._state['U'][2][1] == self._state[
			'U'][1][2] == 'w':
			return False
		...
		return True
	#Checks if there is a full white face and that the first layer is complete by comparing with
	#first layer of solved state
	...
	def is_stage_finish_yellow_cross(self):
		...
		if not self._state['D'][1][0] == self._state['D'][0][1] == self._state['D'][2][1] == self._state[
			'D'][1][2] == 'y':
			return False

		return True
	#Similar to first stage, checks if white cross is finished with its adjacent pieces also being in the right place
	...
	#Checks if yellow corners are in correct position but does not have to be in correct orientation
	def is_stage_finish_position_yellow_corners(self):
		...
		if (not self._state['F'][2][0] in ['r', 'g', 'y'] or not self._state['F'][2][2] in ['r', 'b', 'y']) or \
				...
				(not self._state['D'][0][0] in ['y', 'g', 'r'] or not self._state['D'][0][2] in ['y', 'r', 'b'] or not
				self._state['D'][2][0] in ['y', 'g', 'o'] or not self._state['D'][2][2] in ['y', 'o', 'b']):
			return False

		if not self._state['D'][1][0] == self._state['D'][0][1] == self._state['D'][2][1] == self._state['D'][1][
			2] == 'y':
			return False
	...
	def get_cur_stage(self):
		...
		# finish white edges
		if not self._state['U'][0][1] == self._state['U'][1][0] == self._state['U'][2][1] == self._state['U'][1][
			2] == 'w':
			return 0
	...
	def solve_single_white_corner(self, face, i, j):
		...
		if face in ['F', 'L', 'B', 'R'] and i == 0:
			...
			#pos_map maps the initial position of the white corner to the correct postion
		...
		self.sequence_algorithm(['D' + act_map[j][0], act_map[j][2][face] + act_map[j][0], 'D' + act_map[j][1],
								 act_map[j][2][face] + act_map[j][1]])
	...
	def find_improper_middle_layer(self):
		...
		for face in ['F', 'L', 'B', 'R']:
			for j in [0, 2]:
				if self._state[face][1][j] != face_to_colour[face] or self._state[fac_map[j][face]][1][2 - j] != face_to_colour[
					fac_map[j][face]]:
		...
		if j != None:
			if j == 0:
				self.sequence_algorithm(
					['D+', face_2 + '+', 'D-', face_2 + '-', 'D-', face_1 + '-', 'D+', face_1 + '+'])
			else:
				# j = 2
				self.sequence_algorithm(
					['D-', face_2 + '-', 'D+', face_2 + '+', 'D+', face_1 + '+', 'D-', face_1 + '-'])
	...
	def find_adjacent_solved_yellow_edges(self):
		...
		for i in range(4):
			for face in ['F', 'L', 'B', 'R']:
				if self._state[face][2][1] == face_to_colour[face] and self._state[fac_map[face]][2][1] == face_to_colour[
					fac_map[face]]:
	...
	def is_yellow_edges_solved(self):
		...
		for i in range(4):
			for face in ['F', 'L', 'B', 'R']:
				if self._state[face][2][1] != face_to_colour[face]:
					return False
	...
	def solve_yellow_edges(self):
		...
		if not self.is_yellow_edges_solved():
			self.sequence_algorithm(
				[face_1 + '+', 'D+', face_1 + '-', 'D+', face_1 + '+', 'D+', 'D+', face_1 + '-', 'D+'])

	# finding improper positioned yellow corners
	# fac_map contains corner positions of the yellow face
	# if yellow corner in incorrect position it is swapped to correct corner which is in face_to_colour
	...
	def solve_position_yellow_corners(self):
		...
		while incorrect_pos:
			self.sequence_algorithm(
				['D+', actions[0] + '+', 'D-', actions[1] + '-', 'D+', actions[0] + '-', 'D-', actions[1] + '+'])
			incorrect_pos = self.find_yellow_corner_with_incorrect_position()
	...
	def solve_orient_yellow_corners(self):
		...
		while self._state['F'][0][1] != face_to_colour['F']:
			self.rotate('U+')
		while self._state['F'][2][1] != face_to_colour['F']:
			self.rotate('D+')
